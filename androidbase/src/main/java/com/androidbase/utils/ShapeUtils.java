...
public final class ShapeUtils {
    ...
    public ShapeUtils setStroke(final int width, final ColorStateList colors) {
        if (mDrawable != null) {
            mDrawable.setStroke(width, colors);
        }
        return this;
    }
     ...
     * @return {@link  ShapeUtils}
     */
    public ShapeUtils setStroke(final int width, @ColorInt final int color, final float dashWidth, final float dashGap) {
        if (mDrawable != null) {
     ...
     * @return {@link  ShapeUtils}
     */
    public ShapeUtils setStroke(final int width, final ColorStateList colors, final float dashWidth, final float dashGap) {
        if (mDrawable != null) {
            mDrawable.setStroke(width, colors, dashWidth, dashGap);
        }
        return this;
    }
     ...
     * 设置渐变角度
     *
     * @param orientation 渐变角度 ( 当 angle = 0 时, 渐变色是从左向右, 然后逆时针方向转, 当 angle = 90 时为从下往上,
     *                    angle 必须为 45 的整数倍 )
     * @return {@link  ShapeUtils}
     */
     ...
     * 设置渐变角度
     *
     * @param angle 渐变角度 ( 当 angle = 0 时, 渐变色是从左向右, 然后逆时针方向转, 当 angle = 90 时为从下往上, angle
     *              必须为 45 的整数倍 )
     * @return {@link  ShapeUtils}
     */
     ...
     * 设置渐变色半径
     *
     * @param gradientRadius 渐变色半径, 当 android:type="radial" 时才使用, 单独使用
