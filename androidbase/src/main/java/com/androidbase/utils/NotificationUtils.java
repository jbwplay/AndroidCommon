...
public class NotificationUtils {
    ...
    public static boolean areNotificationsEnabled() {
        return NotificationManagerCompat.from(AndroidUtils.getContext())
     ...
     * Post a notification to be shown in the status bar.
     *
     * @param tag           A string identifier for this notification.  May be {@code
     *                      null}.
     * @param id            An identifier for this notification.
     * @param channelConfig The notification channel of config.
     * @param consumer      The consumer of create the builder of notification.
    ...
    public static void notify(String tag, int id, ChannelConfig channelConfig, Consumer<NotificationCompat.Builder> consumer) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            // Android 8.0+通知栏需要进行设置Channel
            NotificationManager nm = (NotificationManager) AndroidUtils.getContext()
                    .getSystemService(Context.NOTIFICATION_SERVICE);
            //noinspection ConstantConditions
            nm.createNotificationChannel(channelConfig.getNotificationChannel());
        }

        NotificationManagerCompat nmc = NotificationManagerCompat.from(AndroidUtils.getContext());

        NotificationCompat.Builder builder = new NotificationCompat.Builder(AndroidUtils.getContext());
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        // 小图标：必须提供,通知栏上小图标,通过setSmallIcon()进行设置
        // builder.setSmallIcon(android.R.drawable.stat_notify_chat);
        // 标题：可选内容
        // builder.setContentTitle("xxxx");
        // 文本：可选内容
        // builder.setContentText("xxxx");
        // 大图标：可选内容,右侧图标,通过setLargeIcon()进行设置
        // builder.setLargeIcon(xxx);
        // 自定义RemoteViews
        ...
        // builder.setOngoing(false)//设置是否为一个正在进行中的通知，这一类型的通知将无法删除
        // builder.setContentIntent(PendingIntent intent)//设置点击通知后操作
        // builder.setDefaults(int defaults)//设置铃声,振动,闪烁用|分隔,常量在Notification里
        // PendingIntent pendingIntent = PendingIntent.getActivity(AndroidUtils.getContext(), id, new Intent(), PendingIntent.FLAG_CANCEL_CURRENT);
        // builder.setContentIntent(pendingIntent);

        // 点击跳转至指定的页面,当APP退出后,按返回键后能返回至首页
    ...
    public static void cancel(String tag, final int id) {
        NotificationManagerCompat.from(AndroidUtils.getContext()).cancel(tag, id);
    }
    ...
    public static void cancel(final int id) {
        NotificationManagerCompat.from(AndroidUtils.getContext()).cancel(id);
    }
    ...
    public static void cancelAll() {
        NotificationManagerCompat.from(AndroidUtils.getContext()).cancelAll();
    }
    ...
    private static void invokePanels(final String methodName) {
        try {
            ...
            Object service = AndroidUtils.getContext().getSystemService("statusbar");
    ...
    public static class ChannelConfig {
        ...
        public static final ChannelConfig DEFAULT_CHANNEL_CONFIG = new ChannelConfig(AndroidUtils
                .getContext()
                .getPackageName(), AndroidUtils.getContext()
                .getPackageName(), IMPORTANCE_DEFAULT);
         ...
         * in {@link NotificationManager.Policy#INTERRUPTION_FILTER_PRIORITY} mode.
         * <p>
         ...
         *
         * <p>The recommended maximum length is 300 characters; the value may be
         ...
         * <p>
         * Only modifiable before the channel is submitted to {@link
         * NotificationManager#createNotificationChannel(NotificationChannel)}, unless the
         * channel is not currently part of a group.
         *
         * @param groupId the id of a group created by {@link NotificationManager#createNotificationChannelGroup)}.
         */
         ...
         *
         * @param importance the amount the user should be interrupted by notifications
         *                   from this channel.
         */
        ...
        /**
         * Sets the notification light color for notifications posted to this channel, if
         * lights are {@link NotificationChannel#enableLights(boolean) enabled} on this
         * channel and the device supports that feature.
         * <p>
         * Only modifiable before the channel is submitted to {@link
         * NotificationManager#createNotificationChannel(NotificationChannel)}.
         */
         ...
         * not, and if so, whether they appear in a redacted form. See e.g. {@link
         ...
         *
         * <p>The recommended maximum length is 40 characters; the value may be truncated
         * if it is too long.
         */
        ...
        /**
         * Sets whether notifications posted to this channel can appear as application
         ...
         * and its audio attributes. Notification channels with an {@link
         * NotificationChannel#getImportance() importance} of at least {@link
         * NotificationManager#IMPORTANCE_DEFAULT} should have a sound.
         * <p>
         * Only modifiable before the channel is submitted to {@link
         * NotificationManager#createNotificationChannel(NotificationChannel)}.
         */
        ...
        /**
         * Sets the vibration pattern for notifications posted to this channel. If the
         * provided pattern is valid (non-null, non-empty), will {@link
         * NotificationChannel#enableVibration(boolean)} enable vibration} as well.
