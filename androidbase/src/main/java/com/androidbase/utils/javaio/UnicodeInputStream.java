...
/**
 * Unicode input stream for detecting UTF encodings and reading BOM characters. Detects
 * following BOMs:
 * <ul>
 * <li>UTF-8</li>
 * <li>UTF-16BE</li>
...
public class UnicodeInputStream extends InputStream {
     ...
     * <p>
     * Detect mode is active when target encoding is not specified. In detect mode, it
     * tries to detect encoding from BOM if exist. If BOM doesn't exist, encoding is not
     * detected.
     * <p>
     * Read mode is active when target encoding is set. Then this stream reads optional
     * BOM for given encoding. If BOM doesn't exist, nothing is skipped.
     */
    ...
    /**
     * Returns detected UTF encoding or <code>null</code> if no UTF encoding has been
     * detected (i.e. no BOM). If stream is not read yet, it will be {@link #init()
     * initalized} first.
     */
    ...
    /**
     * Detects and decodes encoding from BOM character. Reads ahead four bytes and check
     * for BOM marks. Extra bytes are unread back to the stream, so only BOM bytes are
     * skipped.
     */
    ...
    /**
     * Returns BOM size in bytes. Returns <code>-1</code> if BOM not found.
     */
